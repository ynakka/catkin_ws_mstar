...
from gPC_toolbox.project_gpc_2_statespace import meangpc_fullstate_noinit, meangpc_fullstate
...
def collision_constraint_2d_noinit(radius, obstacle_state, Xprev, num_states, num_uncert, polynomial_degree ):
    mean_states = meangpc_fullstate_noinit(Xprev,num_states,num_uncert,polynomial_degree)
    mean = mean_states.reshape((num_states,1))
    G = np.zeros((num_states,num_states)) # matrix to  pulling out position 
    G[0,0] = 1
